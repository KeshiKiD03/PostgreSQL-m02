...
training=# SELECT count(oficina), region, ciudad FROM oficinas WHERE ciudad ILIKE '%a%' GROUP BY region, ciudad;
 count | region |   ciudad    
-------+--------+-------------
     1 | Este   | Chicago
     1 | Este   | Atlanta
...
training=# SELECT count(num_pedido), rep, importe FROM pedidos GROUP BY rep, importe HAVING importe  > 10000;
 count | rep | importe  
-------+-----+----------
     1 | 106 | 31500.00
     1 | 101 | 22500.00
