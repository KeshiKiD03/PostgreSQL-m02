...
training=# select num_empl, nombre, oficina_rep from repventas where (oficina_rep = 11 or oficina_rep = 12 or oficina_rep = 22) and (contrato >= '1988-06-01' and director is null or cuota < ventas and ventas <= 600000);
 num_empl |   nombre    | oficina_rep 
----------+-------------+-------------
      109 | Mary Jones  |          11
      106 | Sam Clark   |          11
...
training=# select id_producto, descripcion from productos where descripcion ilike 'art%'; id_producto |   descripcion   
-------------+-----------------
...
training=# select fecha_pedido, cant, clie, importe from pedidos where (cant > 20 and importe > 1000) and (clie = '2102' or clie = '2106' or clie = '2109');
 fecha_pedido | cant | clie | importe  
--------------+------+------+----------
 1989-10-12   |   34 | 2102 |  3978.00
 1990-03-02   |   22 | 2109 | 31350.00
 1989-01-04   |   24 | 2106 |  1896.00
(3 rows)

-- 2.43- Identificador dels clients que el seu nom no conté " Corp." o " Inc." amb crèdit major a 30000.

training=# select num_clie, empresa, limite_credito from clientes where (empresa not like '%Corp.%' and empresa not like '%Inc.%') and (limite_credito > 30000);
