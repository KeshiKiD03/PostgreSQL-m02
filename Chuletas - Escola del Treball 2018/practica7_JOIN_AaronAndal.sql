...
-- 4.2- Llista totes les comandes mostrant el seu número, import, número de client i límit de crèdit.
	training=# SELECT num_pedido , importe , clie , limite_credito FROM pedidos , clientes WHERE num_clie = clie;
...
-- 4.4- Llista el nom de tots els clients amb el nom del representant de vendes assignat.
	training=# SELECT empresa , nombre FROM clientes , repventas WHERE rep_clie = num_empl;
      empresa      |    nombre     
-------------------+---------------
 JCP Inc.          | Paul Cruz
...
-- 4.5- Llista la data de totes les comandes amb el numero i nom del client de la comanda.
	training=# SELECT fecha_pedido , clie , empresa FROM pedidos , clientes WHERE num_clie = clie;
...
-- 4.6- Llista les oficines, noms i títols del seus directors amb un objectiu superior a 600.000.
	training=# SELECT oficina , ciudad , nombre , titulo FROM oficinas , repventas WHERE (dir = num_empl) and objetivo > 600000;
...
-- 4.7- Llista els venedors de les oficines de la regió est.
	training=# SELECT num_empl , nombre , oficina, ciudad FROM repventas , oficinas WHERE (dir = num_empl) and region = 'Este';
...
--		i el nom del client que el va sol·licitar.
	training=# SELECT pedidos.* , nombre , empresa FROM pedidos , repventas , clientes 
...
--		nom del venedor que té assignat el client.
	training=# SELECT pedidos.* , empresa , nombre  FROM pedidos , repventas , clientes 
...
--		 el venedor associat al client, i l'oficina on el venedor treballa.
	training=# SELECT pedidos.* , empresa , nombre , oficina_rep  FROM pedidos , repventas , clientes 
	WHERE (clie = num_clie) and (rep_clie = num_empl) and importe > 25000;
...
-- 4.11- Llista totes les combinacions de venedors i oficines on les ventes del venedor és superior a les ventes de l'oficina.
	training=# SELECT num_empl , oficina , repventas.ventas , oficinas.ventas FROM repventas , oficinas 
...
-- 4.12- Informa sobre tots els venedors i les oficines en les que treballen.
	training=# SELECT repventas.* , oficinas.* FROM repventas , oficinas WHERE oficina = oficina_rep;
...
-- 4.13- Llista els venedors amb una quota superior a la dels seus directors.
	training=# SELECT * FROM repventas treb , repventas boss 
...
-- 4.14- Llistar el nom de l'empresa i totes les comandes fetes pel client 2103.
	training=# SELECT empresa , pedidos.* FROM clientes , pedidos WHERE num_clie=clie and clie=2103;
...
--		 els imports i les descripcions del producte.
	training=# SELECT num_pedido , importe , descripcion FROM pedidos , productos 
...
--		 i l'oficina on el venedor treballa. També cal mostar la descripció del producte.
	training=# SELECT empresa , rep , oficina_rep , descripcion , oficinas.ciudad 
...
--		 Per cada comanda mostrar un cop el número, import i data de la comanda.
	training=# SELECT DISTINCT num_pedido , importe , fecha_pedido , contrato FROM pedidos , repventas 
...
--		 amb la ciutat i l'objectiu de l'oficina de cada venedor.
	training=# SELECT nombre , repventas.ventas , ciudad , objetivo FROM repventas , oficinas 
...
--		 el nom del director amb "director".
	training=# SELECT treb.nombre as "empleado", boss.nombre as "director" FROM repventas treb , repventas boss 
...
-- 4.20- Llista totes les combinacions possibles de venedors i ciutats.
	training=# SELECT repventas.num_empl , repventas.oficina_rep , oficinas.oficina , oficinas.ciudad FROM repventas , oficinas;
...
-- 4.21- Per a cada venedor, mostrar el nom, les vendes i la ciutat de l'oficina en cas de tenir-ne una d'assignada.
	training=# SELECT repventas.nombre , repventas.ventas , oficinas.ciudad FROM oficinas , repventas 
...
--		 identificador del fabricant i l'identificador de producte de la comanda.
	training=# SELECT pedidos.num_pedido , pedidos.fecha_pedido , clientes.empresa , productos.id_fab , productos.id_producto 
	FROM pedidos , clientes , productos 
	WHERE pedidos.clie = clientes.num_clie and pedidos.fab = productos.id_fab and pedidos.producto = productos.id_producto 
	and productos.existencias < 10;
...
--		 en cas de tenir oficina assignada.
	training=# SELECT pedidos.num_pedido , pedidos.importe , productos.precio , clientes.empresa , pedidos.rep , oficinas.ciudad 
	FROM pedidos 
...
--		 i el nom del representant de vendes que va realitzar la comanda com a "rep_pedido".
	training=# SELECT pedidos.num_pedido , clientes.empresa , clientes.rep_clie as "rep_cliente" , pedidos.rep as "rep_pedido"
	FROM PEDIDOS 
	JOIN clientes ON pedidos.clie = clientes.num_clie 
...
--		 Mostrar l'identificador de la comanda, el nom del client i el nom del representant de vendes que va prendre la comanda.
	training=# SELECT pedidos.num_pedido , clientes.empresa , repventas.nombre 
	FROM pedidos 
	JOIN clientes ON pedidos.clie = clientes.num_clie 
	JOIN repventas ON pedidos.rep = repventas.num_empl 
