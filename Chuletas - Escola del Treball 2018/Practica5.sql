Base de dades Practica 5
Nom: Fran Cuno

-- 1.- Quantes oficines tenim a cada regió?
training=# SELECT region, count(*) AS "oficines per region" FROM oficinas GROUP BY region;
 region | oficines per region 
--------+---------------------
 Este   |                   3
 Oeste  |                   2
(2 rows)
...
training=# SELECT director, count(nombre) AS "repventas assignats" FROM repventas WHERE director IS NOT NULL GROUP BY director;
...
training=# SELECT oficina_rep, sum(cuota) AS "suma de quotas", avg(cuota) AS "mitjana" FROM repventas WHERE oficina_rep IS NOT NULL GROUP BY oficina_rep;
 oficina_rep | suma de quotas |       mitjana       
-------------+----------------+---------------------
...
training=# SELECT oficina_rep, sum(cuota) AS "suma de quotas", round(avg(cuota), 2) AS "mitjana" FROM repventas WHERE oficina_rep IS NOT NULL GROUP BY oficina_rep;
 oficina_rep | suma de quotas |  mitjana  
-------------+----------------+-----------
...
training=# SELECT oficina_rep max(cuota) AS "Quota" FROM repventas;
 Representant |   Quota   
--------------+-----------
           22 | 350000.00
(1 row)
...
training=# SELECT rep_clie, count(num_clie) FROM clientes GROUP BY rep_clie ORDER BY rep_clie;
 rep_clie | count 
----------+-------
      101 |     3
      102 |     4
...
training=# SELECT descripcion, count(id_producto) FROM productos GROUP BY descripcion ORDER BY Descripcion;
    descripcion    | count 
-------------------+-------
 Ajustador         |     1
 Articulo Tipo 1   |     1
