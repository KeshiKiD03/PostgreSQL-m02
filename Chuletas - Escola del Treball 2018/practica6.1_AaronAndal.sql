...
training=# SELECT MAX(cuota), MIN(cuota), oficina_rep FROM repventas GROUP BY oficina_rep;
    max    |    min    | oficina_rep 
-----------+-----------+-------------
           |           |            
...
training=# SELECT count(num_empl), oficina_rep FROM repventas GROUP BY oficina_rep HAVING oficina_rep IS NOT NULL;
 count | oficina_rep 
-------+-------------
     1 |          13
     1 |          22
...
training=# SELECT count(num_clie), rep, clie FROM clientes GROUP BY rep, clie;
 count | rep_clie 
-------+----------
     2 |      106
     3 |      103
...
training=# SELECT SUM(importe), rep, clie FROM pedidos GROUP BY rep, clie;
   sum    | rep | clie 
----------+-----+------
 31350.00 | 107 | 2109
 47925.00 | 108 | 2112
...
training=# SELECT SUM(importe), rep, clie FROM pedidos GROUP BY rep, clie ORDER BY clie, rep;
   sum    | rep | clie 
----------+-----+------
  1458.00 | 106 | 2101
  3978.00 | 101 | 2102
...
training=# SELECT SUM(importe), rep FROM pedidos GROUP BY rep ORDER BY rep;
   sum    | rep 
----------+-----
 26628.00 | 101
 22776.00 | 102
...
training=# SELECT avg(importe), rep FROM pedidos GROUP BY rep HAVING avg(importe) > 30000;
 avg | rep 
-----+-----
(0 rows)
